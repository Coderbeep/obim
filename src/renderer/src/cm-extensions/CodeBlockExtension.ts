...
const tokenFormattingClasses = {
  ...
  FencedCodeBegin: (isActive: boolean) => {
    return Decoration.line({
      class: `cm-formatting-codeblock-line-begin ${isActive ? "active" : ""}`,
    });
  },
  FencedCodeEnd: (isActive: boolean) => {
    return Decoration.line({
      class: `cm-formatting-codeblock-line-end ${isActive ? "active" : ""}`,
    });
  },
  ...
  class {
    ...
    computeDecorations(view: EditorView) {
      ...
      syntaxTree(view.state).iterate({
        enter: (node) => {
          if (
            node.name === tokenElements.InlineCode ||
            node.matchContext([tokenElements.InlineCode])
          ) {
            if (node.name === tokenElements.CodeMark) {
              builder.add(
                node.from,
                node.to,
                tokenFormattingClasses.MarkHidden,
              );
            }
            ...
            if (node.name === tokenElements.InlineCode) {
              builder.add(
                node.from,
                node.to,
                tokenFormattingClasses.InlineCodeText,
              );
              if (from <= node.to && to >= node.from) return false;
            }
          }
          ...
          if (node.name === "FencedCode") {
            ...
            builder.add(
              node.from,
              node.from,
              tokenFormattingClasses.FencedCodeBegin(isActive),
            );

            if (!isActive) {
              const language = view.state
                .sliceDoc(node.from, lineStarts[0] - 1)
                .slice(3)
                .trim();
              builder.add(
                node.from,
                lineStarts[0] - 1,
                Decoration.widget({
                  widget: new CodeLanguageIndicatorWidget(
                    language,
                    view.state.sliceDoc(
                      lineStarts[0],
            ...
            for (let i = 0; i < lineStarts.length - 1; i++) {
              builder.add(
                lineStarts[i],
                lineStarts[i],
                tokenFormattingClasses.FencedCode,
              );
            }
            builder.add(
              lineStarts[lineStarts.length - 1],
...
class CodeLanguageIndicatorWidget extends WidgetType {
  ...
  private _createCopyButton = () => {
    ...
    const icon = createElement(Clipboard, {
      color: "currentColor",
      width: 16,
      height: 16,
    });
    ...
    button.onclick = () => {
      navigator.clipboard.writeText(this.code).then(() => {
        const icon = createElement(ClipboardCheck, {
          color: "currentColor",
          width: 16,
          height: 16,
        });
